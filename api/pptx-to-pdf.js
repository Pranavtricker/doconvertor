...
export default async function handler(req, res) {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        try {
            ...
            const result = await convertapi.convert('pdf', { File: f.filepath }, 'pptx')
            ...
            res.setHeader('Content-Disposition', 'attachment; filename="' + (f.originalFilename || 'presentation').replace(/\.(pptx|ppt)$/i, '') + '.pdf"')
            res.end(buf)
